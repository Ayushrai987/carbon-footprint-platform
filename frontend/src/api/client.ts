...
client.interceptors.response.use(
  ...
  async (error: AxiosError<ApiResponse<unknown>>) => {
    ...
    if (
      ...
      if (refreshToken) {
        try {
          const res = await axios.post<ApiResponse<{ tokens: AuthTokens }>>(
            "/api/v1/auth/refresh",
            { refreshToken },
          );
          if (res.data.data) {
...
export const footprint = {
  log: (data: LogActivityRequest) =>
    client
      .post<ApiResponse<{ record: FootprintRecord }>>("/footprint/log", data)
      .then((r) => r.data),

  getToday: () =>
    client
      .get<ApiResponse<{ records: FootprintRecord[] }>>("/footprint/today")
      .then((r) => r.data),

  getWeek: () =>
    client
      .get<ApiResponse<{ records: FootprintRecord[] }>>("/footprint/week")
      .then((r) => r.data),

  getMonth: () =>
    client
      .get<ApiResponse<{ records: FootprintRecord[] }>>("/footprint/month")
      .then((r) => r.data),

  getHistory: (page = 1, limit = 20) =>
    client
      .get<
        ApiResponse<{ records: FootprintRecord[] }> & {
  ...
  delete: (id: number) =>
    client
      .delete<ApiResponse<{ message: string }>>(`/footprint/${id}`)
      .then((r) => r.data),
};
...
export const stats = {
  getSummary: () =>
    client
      .get<ApiResponse<StatsResponse>>("/stats/summary")
      .then((r) => r.data),

  getBreakdown: (period = "month") =>
    client
      .get<ApiResponse<BreakdownResponse>>(`/stats/breakdown?period=${period}`)
      .then((r) => r.data),

  getTrend: (period = "month") =>
    client
      .get<ApiResponse<TrendResponse>>(`/stats/trend?period=${period}`)
      .then((r) => r.data),

  getComparison: () =>
    client
      .get<ApiResponse<ComparisonResponse>>("/stats/comparison")
      .then((r) => r.data),
};
